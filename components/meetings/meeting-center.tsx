...
function MeetingWorkspace({ meeting, data, pending, onMutate }: { meeting: MeetingView; data: MeetingCenterData; pending: boolean; onMutate: (mutation: MeetingMutation) => void }) {
  return <article className="meeting-workspace">
    ...
    <div className="meeting-panels">
      ...
      <section><p className="eyebrow">Decide</p><h3>First-class decisions</h3><div className="decision-list">{meeting.decisions.length ? meeting.decisions.map((decision) => <article className="decision-card" key={decision.id}><div><Gavel aria-hidden="true" /><span>{decision.status}</span></div><strong>{decision.statement}</strong><p>{decision.rationale}</p><small>{decision.authorityName} · {decision.decidedLabel} · Review: {decision.reviewTrigger}</small></article>) : <p className="panel-empty">No decisions recorded from this meeting.</p>}</div><DecisionForm meeting={meeting} pending={pending} onMutate={onMutate} /></section>
...
function MeetingPlanForm({ meeting, pending, onMutate }: { meeting: MeetingView; pending: boolean; onMutate: (mutation: MeetingMutation) => void }) {
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const values = new FormData(event.currentTarget); onMutate({ action: 'UPDATE_MEETING', meetingId: meeting.id, title: String(values.get('title')), purpose: String(values.get('purpose')), agenda: String(values.get('agenda')), sharedSummary: String(values.get('sharedSummary') ?? ''), followUp: String(values.get('followUp') ?? ''), phase: String(values.get('phase')) as MeetingView['phase'] }); }
  const currentIndex = meetingPhases.indexOf(meeting.phase);
  return <form className="meeting-form plan-form" onSubmit={submit}><label>Title<input name="title" defaultValue={meeting.title} required /></label><label>Purpose<textarea name="purpose" defaultValue={meeting.purpose} rows={2} required /></label><label>Agenda<textarea name="agenda" defaultValue={meeting.agenda} rows={5} required /></label><label>Shared summary<textarea name="sharedSummary" defaultValue={meeting.sharedSummary} rows={3} placeholder="What may participants carry forward together?" /></label><label>Follow-up<textarea name="followUp" defaultValue={meeting.followUp} rows={3} placeholder="Unresolved items, next review, or next meeting." /></label><label>Workflow stage<select name="phase" defaultValue={meeting.phase}>{meetingPhases.filter((_, index) => index === currentIndex || index === currentIndex + 1).map((phase) => <option value={phase} key={phase}>{phase.replace('_', ' ')}</option>)}</select></label><button className="secondary-button" disabled={pending} type="submit">Save meeting plan</button></form>;
}

function DecisionForm({ meeting, pending, onMutate }: { meeting: MeetingView; pending: boolean; onMutate: (mutation: MeetingMutation) => void }) {
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = event.currentTarget; const values = new FormData(form); onMutate({ action: 'ADD_DECISION', meetingId: meeting.id, statement: String(values.get('statement')), rationale: String(values.get('rationale')), intendedEffect: String(values.get('intendedEffect')), reviewTrigger: String(values.get('reviewTrigger')) }); form.reset(); }
  return <form className="meeting-form decision-form" onSubmit={submit}><label className="wide">Decision<input name="statement" required placeholder="What has an authorized participant decided?" /></label><label className="wide">Rationale<textarea name="rationale" rows={2} required placeholder="Why is this choice being made?" /></label><label>Intended effect<input name="intendedEffect" required placeholder="What should this decision enable?" /></label><label>Review trigger<input name="reviewTrigger" required placeholder="When should it be reconsidered?" /></label><button className="secondary-button" disabled={pending} type="submit"><Gavel aria-hidden="true" />Record decision</button></form>;
}
