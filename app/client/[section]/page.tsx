...
import { CLIENT_MCP_PATH, mcpOAuthReadiness, mcpResourceUrl } from '@/lib/mcp/configuration';
...
export default async function ClientSection({ params }: { params: Promise<{ section: string }> }) {
  ...
  if (section === 'settings') {
    ...
    return <><PageIntro eyebrow="Client portal" title={copy.title} description="Manage your secure identity and connect the AI assistant you use for your assigned Lead Emergence work." /><EntryIdentityConnection /><McpConnectionCenter audience="client" endpoint={mcpResourceUrl(requestOrigin, CLIENT_MCP_PATH).toString()} readiness={readiness} grants={grants} /></>;
  }
