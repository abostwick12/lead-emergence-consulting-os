...
import { CLIENT_MCP_PATH, mcpResourceUrl } from '@/lib/mcp/configuration';
import { createLeadEmergenceClientMcpServer } from '@/lib/mcp/client-server';
...
const handler = createMcpHandler(({ authInfo }) => {
  ...
  return createLeadEmergenceClientMcpServer(authInfo);
}, { legacy: 'stateless', responseMode: 'json', onerror: (error) => console.error('[mcp.client.protocol]', error) });

async function serve(request: Request) {
  const requestOrigin = new URL(request.url).origin;
  const resourceUrl = mcpResourceUrl(requestOrigin, CLIENT_MCP_PATH);
  ...
  const gate = requireBearerAuth({ verifier: new SupabaseMcpTokenVerifier(requestOrigin, CLIENT_MCP_PATH), resourceMetadataUrl });
