...
export function validateDiscoveryMutation(value: unknown): DiscoveryMutation {
  if (!value || typeof value !== 'object') throw new Error('A discovery intake action is required.');
  const input = value as Record<string, unknown>;
  const required = (key: string) => { const field = input[key]; if (typeof field !== 'string' || !field.trim()) throw new Error(`${key} is required.`); return field.trim(); };
  if (input.action === 'CAPTURE_EVIDENCE') {
    const sourceType = required('sourceType') as EvidenceSourceType; if (!evidenceTypes.includes(sourceType)) throw new Error('Evidence source type is invalid.');
  ...
  if (input.action === 'CREATE_ASSESSMENT') {
    const confidentiality = required('confidentiality') as AssessmentConfidentiality; if (!confidentialityValues.includes(confidentiality)) throw new Error('Assessment confidentiality is invalid.');
