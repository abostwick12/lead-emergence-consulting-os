...
export interface EngagementProduct { id: string; name: string; description: string; ownerLabel: string; status: 'ACTIVE' | 'ON_HOLD' | 'COMPLETE'; handlingLabel: string }
export interface AuditAssignment { id: string; productId: string; title: string; respondentLabel: string; dueOn: string; status: WorkStatus; completedResponses: number; totalPrompts: number }
export interface InterviewPlan { id: string; productId: string; participantLabel: string; interviewType: string; objective: string; scheduledFor: string; status: 'PLANNED' | 'IN_PROGRESS' | 'COMPLETED'; notesCount: number }
