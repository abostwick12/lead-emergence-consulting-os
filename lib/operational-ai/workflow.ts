...
export function validateOperationalMutation(value: unknown): OperationalMutation {
  if (!value || typeof value !== 'object') throw validationError('A workspace action is required.');
  ...
  if (action === 'SAVE_GUIDED_RESPONSE') {
    ...
    if (!guidedQuestions[recordKind].some((question) => question.id === questionId)) throw validationError('questionId is not valid for this guided record type.');
  ...
  throw validationError('Unsupported workspace action.');
}
