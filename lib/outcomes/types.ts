...
export interface OutcomesNewRealityData {
  ...
  evaluation?: { harvest: string; soil: string; dimensions: Array<{ name: string; rating: string; rationale: string }> };
...
export type OutcomesMutation =
  | { action: 'RECORD_OUTCOME'; measuredValue: string; statement: string }
  | { action: 'EVALUATE_VALUE'; harvest: string; soil: string }
  | { action: 'VALIDATE_LEARNING'; statement: string; disposition: OutcomeDisposition }
