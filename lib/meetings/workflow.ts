...
export function validateMeetingMutation(value: unknown): MeetingMutation {
  if (!value || typeof value !== 'object') throw validationError('A meeting action is required.');
  const input = value as Record<string, unknown>;
  if (typeof input.action !== 'string') throw validationError('A meeting action is required.');
  ...
  switch (input.action) {
    case 'CREATE_MEETING': {
      ...
      if (meetingType !== 'CONSULTING' && meetingType !== 'COACHING') throw validationError('Meeting type is invalid.');
    ...
    case 'UPDATE_MEETING': {
      ...
      if (!meetingPhases.includes(phase)) throw validationError('Meeting phase is invalid.');
    ...
    case 'ADD_PRIVATE_NOTE': {
      ...
      if (kind !== 'CONSULTANT_NOTE' && kind !== 'INDIVIDUAL_REFLECTION') throw validationError('Private note kind is invalid.');
    ...
    case 'UPDATE_COMMITMENT': {
      ...
      if (!['OPEN', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED'].includes(status)) throw validationError('Commitment status is invalid.');
    ...
    default:
      throw validationError('Meeting action is not supported.');
  }
}
