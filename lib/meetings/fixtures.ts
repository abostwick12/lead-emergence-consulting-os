...
export function mutateFixtureMeeting(session: PortalSession, mutation: MeetingMutation): MeetingCenterData {
  ...
  if (mutation.action === 'CREATE_MEETING') {
    ...
    const meeting: MeetingView = {
      ...
      participants: [consultant, participant].filter((item, index, items) => items.findIndex((candidate) => candidate.id === item.id) === index), notes: [], commitments: [], context: [],
    ...
    if (mutation.action === 'UPDATE_MEETING') {
      ...
      Object.assign(meeting, { title: mutation.title, purpose: mutation.purpose, agenda: mutation.agenda, phase: mutation.phase, status: mutation.phase === 'PREPARE' ? 'PREPARED' : mutation.phase === 'FOLLOW_UP' ? 'COMPLETED' : 'IN_PROGRESS' });
